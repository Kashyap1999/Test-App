...
export default function AttributesPage() {
  ...
  const handleSave = (id) => {
    fetcher.submit({ id, name: editName, groups: JSON.stringify(selectedGroups), action: "update" }, { method: "post" });
    setAttributes(attributes.map(attribute =>
      attribute.id === id ? { ...attribute, name: editName, groups: selectedGroups } : attribute
    ));
    setEditingId(null);
  };
      ...
      <BlockStack gap="500">
        <Card title="Online store dashboard">
          <Text>
            <Banner>
              <p>
                Manage your attribute groups efficiently. You can add, edit, and delete groups to
                better organize your attributes. <Link url="/app/groups">Manage Groups</Link>
              </p>
            </Banner>
          </Text>
        </Card>
        ...
        <Card roundedAbove="sm" padding="500">
          <InlineStack align="start" gap="300">
            <div style={{ flexGrow: 1 }}>
              <AutocompleteExample attributes={attributes} setAttributes={setAttributes} allAttributes={initialAttributes} />
            </div>
            <Button variant="primary" icon={PlusIcon} onClick={() => setIsAddingAttribute(true)}>
              Add Attribute
            </Button>
          </InlineStack>
        </Card>
                ...
                <InlineStack gap="300">
                  <div style={{ display: "flex", gap: "10px" }}>
                    <TextField
                      ...
                      autoComplete="off"
                    />
                    <GroupSelector groups={groupsloader} onGroupSelect={handleGroupSelection} selectedGroups='' />
                  </div>
                  ...
                  <div style={{ display: "flex", gap: "10px" }}>
                    ...
                    <div>
                      <Button variant="secondary" onClick={() => setIsAddingAttribute(false)}>Cancel</Button>
                    </div>
                  </div>
                </InlineStack>
              </fetcher.Form>
        ...
        <Layout gap="500">
          <Layout.Section>
            <Card gap="300">
              {attributes.length > 0 ? (
                <ResourceList
                  resourceName={{ singular: "manage attributes", plural: "manage attributes" }}
                  items={attributes}
                  renderItem={(item) => {
                    ...
                    return (
                        ...
                        shortcutActions={[
                          {
                            ...
                            onAction: () => handleEditClick(id, name, groupsloader)
                          },
                          {
                      ...
                      >
                        <div style={{ display: "flex", alignItems: "center", gap: "6px" }}>
                          {editingId === id ? (
                            <>
                              <TextField value={editName} onChange={(value) => setEditName(value)} autoComplete="off" />
                              <GroupSelector groups={groupsloader} onGroupSelect={handleGroupSelection} selectedGroups={groups} />
                              <input type="hidden" name="groups" value={JSON.stringify(selectedGroups)} />
                ...
                <EmptyState
                  ...
                  action={{ content: "Add Attribute", onAction: () => setIsAddingAttribute(true) }}
...
export async function loader() {
  ...
  return new Response(JSON.stringify({ attributes, groupsloader }), { headers: { "Content-Type": "application/json" } });
}
...
export async function action({ request }) {
  ...
  try {
    ...
    return new Response(JSON.stringify({ attributes }), { headers: { "Content-Type": "application/json" } });
  } catch (error) {
...
function AutocompleteExample({ attributes = [], setAttributes, allAttributes }) {
  const [inputValue, setInputValue] = useState("");
  const [options, setOptions] = useState([]);

  // useEffect(() => {
  //   if (Array.isArray(attributes)) {
  //     setOptions(attributes.map((attribute) => ({ value: attribute.id, label: attribute.name })));
  //   }
  // }, [attributes]);
  ...
  const updateText = (value) => {
    ...
    if (value === "") {
      ...
      setOptions(
        value
          ? attributes
            .filter((attribute) => attribute.name?.toLowerCase().includes(value.toLowerCase()))
  ...
  const handleSelect = (selected) => {
    const selectedAttributes = allAttributes.find((attribute) => attribute.id === selected[0]);
    if (selectedAttributes) {
      setAttributes([selectedAttributes]); // Update ResourceList to only show selected group
      setInputValue(selectedAttributes.name); // Update input value
    }
  };
...
export function GroupSelector({ groups = [], onGroupSelect, selectedGroups = [] }) {
  const [selectedGroupIds, setSelectedGroupIds] = selectedGroups ? useState(selectedGroups.map(group => group.id)) : useState(''); // Initialize selected groups
  ...
  const toggleGroupSelection = useCallback(
    (groupId) => {
      setSelectedGroupIds((prevSelected) => {
        ...
        if (onGroupSelect) {
          const selectedGroups = groups.filter((group) => updatedSelection.has(group.id));
  ...
  const filteredGroups = useMemo(() => {
    ...
    return groups.filter((group) => group.name.toLowerCase().includes(searchValue.toLowerCase()));
  }, [searchValue, groups]);
        ...
        {filteredGroups.length > 0 ? (
          <Listbox onSelect={toggleGroupSelection}>
            {filteredGroups.map((group) => (
              <Listbox.Option key={group.id} value={group.id} selected={selectedGroupIds.includes(group.id)}>
                <Listbox.TextOption selected={selectedGroupIds.includes(group.id)}>
                  {group.name}
                </Listbox.TextOption>
              </Listbox.Option>
            ))}
